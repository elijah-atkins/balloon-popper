...
class App {
    ...
    resetBalloon(balloon) {
        ...
        const color = this.colors[Math.floor(Math.random() * this.colors.length)];
        balloon.position.set(Math.sin(index + frame * 5) * 10 - 5, -Math.sin(index + frame * 10) * 10 - 15, 0);
    ...
    checkCollision(balloon) {
        ...
        for (let i = 0; i < this.scene.children.length; i++) {
            ...
            if (this.scene.children[i].visible) {
                //check if the balloon is not the current balloon
                if (i != index) {
                    //check if the distance between the current balloon and the other balloon is less than 1
